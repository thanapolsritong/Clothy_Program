...
export default function OutfitDetail() {
  ...
  const otherOutfits = outfits.filter(o => o.customerId === customerId && o.id !== outfitId);

  useEffect(() => {
    if (outfit && outfit.measurements) {
      setMeasurements(outfit.measurements);
    }
  }, [outfit]);
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#F9F8F6]">
        <div className="text-center">
          ...
          <button onClick={() => navigate('/')} className="text-[#8F9779] underline hover:text-[#7a8264]">กลับไปยังหน้าแรก</button>
        </div>
      </div>
    );
  }
  ...
  const handleCopyMeasurements = (sourceOutfitId: string) => {
    const sourceOutfit = outfits.find(o => o.id === sourceOutfitId);
    if (sourceOutfit && sourceOutfit.measurements) {
      if (window.confirm(`ต้องการคัดลอกสัดส่วนจาก "${sourceOutfit.name}" ใช่หรือไม่?\n(ข้อมูลวันที่และราคาจะไม่ถูกคัดลอกมาด้วย)`)) {
        setMeasurements(prev => ({
          ...prev, 
          ...sourceOutfit.measurements,
  ...
  const handleSaveMeasurements = (e: React.FormEvent) => {
    e.preventDefault();
    updateMeasurements(outfit.id, measurements);
    setIsSaved(true);
  ...
  const handleSaveUpload = async () => {
    if (selectedImage) {
      await addPhoto(outfit.id, selectedImage, newCaption);
    ...
    <div>
      ...
      <div className="relative">
        <input
          ...
          className="w-full px-3 py-2 bg-[#F9F8F6] border border-[#e5e5e0] focus:border-[#8F9779] focus:bg-white rounded-lg outline-none transition-all text-sm pr-8"
  ...
  return (
    <div className="min-h-screen bg-[#F9F8F6] text-[#333333] font-sans pb-20">
      <div className="max-w-5xl mx-auto px-6 py-10">
        ...
        <header className="mb-8 flex flex-col sm:flex-row justify-between items-start sm:items-end border-b border-[#e5e5e0] pb-6 gap-6">
          <div>
            <button 
              onClick={() => navigate(-1)} 
              className="flex items-center text-gray-500 hover:text-[#8F9779] transition-colors mb-4 group text-sm font-medium"
            >
              <ChevronLeft size={18} className="mr-1 group-hover:-translate-x-1 transition-transform" /> 
              กลับไป
            </button>
            <h1 className="text-3xl font-light text-[#333333] mb-2">{outfit.name}</h1>
            <p className="text-gray-500 text-sm">ลูกค้า: <span className="font-medium text-gray-700">{customer.name}</span></p>
          </div>
          ...
          <div className="flex gap-3 items-center print:hidden">
            ...
            <div className="bg-white p-2 rounded-xl shadow-sm border border-[#e5e5e0] flex">
              {statusOptions.map((status) => (
                <button
                  key={status}
                  onClick={() => updateOutfitStatus(outfit.id, status)}
        ...
        {/* ส่วนที่ 1: รายละเอียดและสัดส่วน (เหมือนเดิม) */}
        <div className="bg-[#8F9779] p-5 text-white flex justify-between items-center flex-wrap gap-4 rounded-t-2xl">
          ...
          {otherOutfits.length > 0 && (
            ...
            >
              ...
              {otherOutfits.map(o => <option key={o.id} value={o.id}>{o.name}</option>)}
            </select>
          )}
        </div>
        ...
        <div className="w-full bg-white rounded-b-2xl shadow-sm border border-[#e5e5e0] border-t-0 overflow-hidden mb-12">
          <form onSubmit={handleSaveMeasurements}>
            ...
            <div className="p-6 bg-[#f4f5f2] border-b border-[#e5e5e0] grid grid-cols-1 md:grid-cols-5 gap-4">
              <div><label className="text-xs font-semibold text-gray-700 mb-1 flex items-center gap-1"><Calendar size={12}/> วันที่รับงาน</label><input type="date" value={measurements.orderDate || ''} onChange={(e) => setMeasurements({ ...measurements, orderDate: e.target.value })} className="w-full px-3 py-2 rounded-md border border-[#d1d1cc] text-sm outline-none focus:border-[#8F9779]" title="วันที่รับงาน" /></div>
              ...
              <div><label className="text-xs font-semibold text-gray-700 mb-1 flex items-center gap-1"><CreditCard size={12}/> ราคา</label><input type="number" value={measurements.price || ''} onChange={(e) => handleFinancialChange('price', e.target.value)} placeholder="0" className="w-full px-3 py-2 rounded-md border border-[#d1d1cc] text-sm outline-none focus:border-[#8F9779]" /></div>
              <div><label className="text-xs font-semibold text-gray-700 mb-1 flex items-center gap-1">มัดจำ</label><input type="number" value={measurements.deposit || ''} onChange={(e) => handleFinancialChange('deposit', e.target.value)} placeholder="0" className="w-full px-3 py-2 rounded-md border border-[#d1d1cc] text-sm outline-none focus:border-[#8F9779]" /></div>
            ...
            <div className="p-6 border-t border-[#e5e5e0] bg-gray-50 flex justify-end">
              <button type="submit" className={`px-8 py-3 rounded-xl flex items-center justify-center font-medium transition-all duration-300 shadow-sm hover:shadow-md focus:outline-none ${isSaved ? 'bg-emerald-600 hover:bg-emerald-700 text-white' : 'bg-[#8F9779] hover:bg-[#7a8264] text-white'}`}>
        ...
        {/* ส่วนที่ 2: รูปภาพอ้างอิง (อัปเกรดใหม่!) */}
        <div className="w-full">
          <div className="flex justify-between items-end mb-6 border-b border-[#e5e5e0] pb-4">
            ...
            >
              <UploadCloud size={18} className="mr-2 text-gray-400 group-hover:text-[#8F9779]" /> 
              เพิ่มรูปภาพ
            </button>
          </div>
            ...
            <div className="mb-8 p-6 bg-white border border-[#e5e5e0] rounded-2xl shadow-sm flex flex-col sm:flex-row gap-6 items-start animate-fade-in">
              ...
              <div className="flex-1 w-full">
                ...
                <textarea 
                  ...
                  className="w-full p-3 rounded-xl bg-[#F9F8F6] border border-[#e5e5e0] outline-none focus:border-[#8F9779] resize-none h-20 mb-3 text-sm"
                />
                <div className="flex gap-2">
                  <button onClick={handleSaveUpload} className="bg-[#8F9779] hover:bg-[#7a8264] text-white px-5 py-2.5 rounded-xl flex items-center text-sm font-medium transition-colors">
          ...
          {outfit.photos.length === 0 && !selectedImage ? (
            <div className="bg-white rounded-2xl border border-dashed border-[#d1d1cc] p-16 text-center shadow-sm">
              ...
              <button
                onClick={() => fileInputRef.current?.click()}
                className="bg-[#8F9779] hover:bg-[#7a8264] text-white px-8 py-3 rounded-xl inline-flex items-center font-medium transition-colors shadow-sm"
              >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {outfit.photos.map((photo) => (
                <div key={photo.id} className="bg-white rounded-2xl overflow-hidden shadow-sm border border-[#e5e5e0] flex flex-col group relative">
                  
                  {/* ปุ่มลบรูปลอยอยู่มุมขวาบน (โชว์ตอนเอาเมาส์ชี้) */}
                  <button 
                    onClick={() => { if(window.confirm('ต้องการลบรูปภาพนี้ใช่หรือไม่?')) deletePhoto(photo.id); }}
                    title="ลบรูปภาพ"
                  ...
                  <div className="aspect-square bg-gray-100 relative overflow-hidden border-b border-[#e5e5e0]">
                    <img src={photo.url} alt="รูปภาพอ้างอิง" className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
                  </div>
                  ...
                  {editingPhotoId === photo.id ? (
                    <div className="p-4 bg-blue-50 flex items-center gap-2">
                      ...
                      />
                      <button onClick={() => { updatePhotoCaption(outfit.id, photo.id, editCaptionText); setEditingPhotoId(null); }} className="text-emerald-600 bg-emerald-100 p-2 rounded-lg hover:bg-emerald-200" title="บันทึก"><Check size={18}/></button>
